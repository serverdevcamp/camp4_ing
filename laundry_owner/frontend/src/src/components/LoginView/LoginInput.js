...
const LoginInput = ({ handleLogin, handleSignUp, handleFindPassword }) => {
  ...
  return (
    <div className={cx('inputContent')}>
      ...
      <form action="#">
        <label> <span className={cx('label')}>ID</span> <input className={cx('inputText')} type={'email'} required />
        </label>
        <label> <span className={cx('label')}>비밀번호</span> <input className={cx('inputText')} type={'password'}
          required /></label>
